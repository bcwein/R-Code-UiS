...
- \left(\tau\sum_{i=1}^n x_i^2 + \frac{1}{100}\right)\beta^2
$$
And we have
...
$$
c_\beta = 
- \left(\tau\sum_{i=1}^n x_i^2 + \frac{1}{100}\right)
$$
...
ESS(m[,3])
```
We see that we have a pretty good sample size. Now let's investigate wether we
...
quantile(alphapbeta, 0.95)
```

```{r}
alphapbeta2 <- m2[,1] + m2[,2]
hist(alphapbeta2, breaks=25)
quantile(alphapbeta2, 0.95)
```
