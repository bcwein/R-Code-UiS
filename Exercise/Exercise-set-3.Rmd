...
simTriangle <- function(x, min, max, mode) {
   return(rtri(x, a = min, b = max, c = mode))
}

simConPx <- function (x, n) {
   nSim <- n
   sims <- simTriangle(nSim, 2, 10, 6)
    return(sum(sims >= x) / nSim)
}

simConPx(8, 10000)
```
## Simulate unconditional probability

```{r}
uncSimTriangle <- function(x, nSim, min, max, mode) {
   size <- vector(length = nSim)
   for(i in 1:Nsim) {
      a <- runif(1)
      if(a < 0.4)
         size[i] <- rtri(1, a = min, b = max, c = mode)
      else
         size[i] <- 0
   }
